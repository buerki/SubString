...
##############################################################################
# length-adjust.sh (c) ABuerki 2014, licensed under the EUPL V.1.1.
version='0.9.6'
####
...
if [ $num_of_regexes -eq 1 ]; then
	...
	for line in $(echo "$input_list" | egrep "$separator\.$relevant_regex($|\.)"); do
		...
		nsize=$(echo "$line" | awk '{c+=gsub(s,s)}END{print c}' s="$separator")
		if [ "$nsize" -gt "2" ]; then
...
if [ "$verbose" == true ]; then
	echo "correcting $(echo "$lines_to_be_adjusted" | awk '{c+=gsub(s,s)}END{print c}' s=' ') n-grams below cutoff frequency..."
fi
...
# files of the name as in the argument or any with the name as the argument
...
# analyse options
while getopts b:hc:d23456789lop:rs:vV opt
do
	case $opt	in
	b)	basetop=$OPTARG
		;;
	...
	V)	echo "$(basename $0)	-	version $version"
		echo "Copyright (c) 2014 Andreas Buerki"
...
for list; do
	if [ -s $list ]; then
		...
		# check list format
		if [ $(head -1 $list | awk -v RS="	" 'END {print NR - 1}') -eq 1 ] ; then
			if [ "$verbose" == true ]; then
				echo "1tab recognised, no document counts"
			fi
		elif [ $(head -1 $list | awk -v RS="	" 'END {print NR - 1}') -eq 2 ] ; then
			if [ "$verbose" == true ]; then
...
if [ -z "$regex" ]; then
	...
	if [ "$(echo $regex | awk '{c+=gsub(s,s)}END{print c}' s=' ')" -le 1 ]; then
...
add_to_name correction-log.txt; corr_log_name=$output_filename
...
for line in $(echo "$input_list" | cut -d '.' -f 1) ; do
		...
		if [ $(echo "$input_list" | egrep "(^$line|$separator$line\.)" | grep -v "^$line\." | wc -l) -ge $activationat ]; then
			...
			# establish nsize + 1
			nsize_plus1=$(( $(echo "$line" | awk '{c+=gsub(s,s)}END{print c}' s="$separator") + 1 ))
			...
			for asso_line in $(echo "$input_list" | egrep "^$line|$separator$line\."); do
				if [ $(echo "$asso_line" | awk '{c+=gsub(s,s)}END{print c}' s="$separator") -eq $nsize_plus1 ]; then
					# count 'em
					(( num_of_asso_lines += 1 ))
...
# write output file
add_to_name $argument_file.adju
