...
##############################################################################
# length-adjust.sh 
copyright="(c) 2016-2018 Cardiff University, 2014 Andreas Buerki"
...
DESCRRIPTION: this script takes substringreduced n-gram lists and checks for
              strings that have an unusually higher number of variants off
              their base. It then offers various correction procedures.
SYNOPSIS:     $(basename $0) -c N [-OPTIONS] FILE[S]
...
# write output file
ext="$(egrep -o '\.[[:alnum:]]+$' <<<"$argument_file")"
new="$(sed "s/$ext//" <<< "$argument_file")"
add_to_name "$new.adju.txt"
