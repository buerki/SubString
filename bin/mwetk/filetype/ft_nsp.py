...
# Sandra Castellanos
#
# ft_nsp.py is designed to work with mwetoolkit
#
...
class NSPInfo(common.FiletypeInfo):
    description = "Ngram Statistics Package-like format (n<>gram<>78)"
    filetype_ext = "NSP"
    ...
    def operations(self):
        return common.FiletypeOperations(NSPChecker, None, NSPPrinter)

INFO = NSPInfo()

################################################################################

class NSPChecker(common.AbstractChecker):
    r"""Checks whether input is in NSP format."""
    def matches_header(self, strict):
        return not strict

################################################################################

class NSPPrinter(common.AbstractPrinter):
    ...
    def __init__(self, ctxinfo, category, freq_source=None,
        ...
        super(NSPPrinter, self).__init__(ctxinfo, category)
    ...
    def handle_meta(self, meta, ctxinfo):
        """Print the header for the NSP dataset file,
        and save the corpus size."""
        #string_cand = "l1\tl2\tf\tN"

    def handle_candidate(self, entity, ctxinfo):
        """Print each `Candidate` as a NSP data set entry line.
