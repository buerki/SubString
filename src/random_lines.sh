...
if [ "$(grep 'CYGWIN' <<< $platform)" ]; then
	if [ "$stout" == "true" ] ; then
		...
		sort --random-sort "$1" | head -$number | sed 's/^[0-9]* //' > "$1.random$number"
	fi
else
	if [ "$stout" == "true" ] ; then
		awk 'BEGIN {srand()} {printf "%05.0f %s \n",rand()*999999, $0; }' "$1" | sort -n | head -$number | sed 's/^[0-9]* //'
	else
		awk 'BEGIN {srand()} {printf "%05.0f %s \n",rand()*999999, $0; }' "$1" | sort -n | head -$number | sed 's/^[0-9]* //' > "$1.random$number"
	fi
	if [ "$verbose" == "true" ] ; then
		echo "$number random lines out of $1 written to $1.random$number"
	fi
fi