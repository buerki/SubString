...
A more in-depth theoretical description and justification of the algorithm is currently in preparation.
...
This script is used to convert n-gram lists into the format required by SubString. listconv.sh is able to convert output lists created with the NGram Statistics Package (Text-NSP, <http://ngram.sourceforge.net>) or those created by NGramTools (http://homepages.inf.ed.ac.uk/lzhang10/ ngram.html or http://morphix-nlp.berlios.de/manual/node28.html).
...
substring.sh takes longer to process the lists this time because the amount of data is larger (lists are based on about 120,000 words of text). The output (the .substrd file) is again placed in the current directory. The output list will not contain any 7-grams because there are none above the chosen cutoff of 9 (the list 7-grams.cut.([0-9]) is in fact empty) and there is only one 6-gram above the cutoff. The consolidation will have produced some n-grams with frequencies below 10. To make sure the list only contains n-grams with a frequency of at least 10, cutoff.sh can be used to remove any n-grams below this frequency.
